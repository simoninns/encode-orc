...
def generate_colorbar_frame_yuv422(width, height, saturation_percent):
    """
    Generate EIA/SMPTE color bars in Y'CbCr 4:2:2 format (YUYV)
    
    Args:
        width: Frame width (must be even, typically 720 for NTSC)
...
def main():
    ...
    # Generate 100% bars
    print(f"Generating NTSC EIA 100% color bars ({width}x{height}, Y'CbCr 4:2:2 YUYV)...")
    ...
    # Generate 75% bars
    print(f"Generating NTSC EIA 75% color bars ({width}x{height}, Y'CbCr 4:2:2 YUYV)...")
    ...
    expected_size = width * height * 2  # 2 bytes per pixel (average) for 10-bit YUYV in 16-bit containers
    expected_size = (width // 2) * height * 4 * 2  # More accurate: (width/2 pairs) * height * 4 components * 2 bytes
