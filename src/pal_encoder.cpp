...
void PALEncoder::enable_vitc(int32_t start_frame_offset) {
    if (!vitc_generator_) {
...
void PALEncoder::disable_vitc() {
    vitc_enabled_ = false;
}

bool PALEncoder::is_vitc_enabled() const {
...
Field PALEncoder::encode_field_from_yuv(const Field& field_yuv,
                                        int32_t field_number, 
                                        bool is_first_field,
                                        const VBIData* vbi_data) {
    ...
    // Now add metadata and active video on top of the structure
    ...
    for (int32_t line = 0; line < field_height; ++line) {
        ...
        // Skip vsync lines - already generated by FieldStructureGenerator
        if (line < VSYNC_LINES) {
            continue;
        }
        // Lines 6-22: VBI (Vertical Blanking Interval)
            ...
            // VITS lines (if enabled)
                ...
                // Second field VITS lines (0-indexed in field)
                    ...
                    // Sync already generated, just add color burst
                    generate_color_burst(line_buffer, line, field_number);
                }
            }
            else if (is_vitc_enabled()) {
                // Consumer tape VITC placement: lines 19 and 21 (0-indexed 18,20)
                // Sync already generated, add color burst
                generate_color_burst(line_buffer, line, field_number);
                if (line == 18 || line == 20) {
        ...
        // Lines 23-310: Active video
            ...
            if (line_in_field < source_field_height) {
                // Get pointers to YUV line data (already split into field)
                ...
                // Sync already generated by structure generator, add color burst
                generate_color_burst(line_buffer, line, field_number);
                ...
                // Beyond source field - sync already present, add burst
                generate_color_burst(line_buffer, line, field_number);
            }
        }
